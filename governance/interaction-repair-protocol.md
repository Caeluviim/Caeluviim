...
1. **Action before narration.** When repeated obstruction is identified and a safe concrete corrective action is available, perform that action before explaining it.
2. **No false completion.** Never describe a draft, workbook, graph ingestion, repository change, filing, test, or runtime state as complete unless corresponding artifact or verification evidence exists.
...
8. **Use governed repository writes.** Material repository changes must be made on a branch and integrated through a pull request when the main-write guard is active; a reverted direct push is not completion.
