...
def build_parser() -> argparse.ArgumentParser:
    ...
    audit.add_argument("--without-catalog", action="store_true", help="Skip manifest catalog binding")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "health":
        ...
        if receipt_audit["status"] != "valid":
            raise RuntimeError(
                "Runtime receipt ledger failed after sync: "
                + json.dumps(receipt_audit, sort_keys=True)
            )
