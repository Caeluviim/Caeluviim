...
def audit_receipts(directory: Path, *, catalog: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    The audit is intentionally fail-closed. A malformed file, invalid receipt hash,
    duplicate receipt hash, duplicate manifest/timestamp event, unsupported receipt
    type, unresolved runtime/source identity, or catalog mismatch invalidates the
    ledger. This proves repository-visible receipt integrity only; it does not prove
    that the referenced runtime still exists or that its current graph matches a
    historical receipt.
    """
    ...
    if catalog is not None:
        catalog_by_ingest = {
            item["ingest_id"]: item for item in catalog.get("manifests", [])
        }
        ...
        try:
            ...
            if catalog is not None:
                ...
                if expected.get("manifest_hash") != manifest_hash:
                    raise ValueError(
                        f"manifest hash mismatch for {ingest_id}: "
                        f"receipt={manifest_hash} catalog={expected.get('manifest_hash')}"
                    )
    ...
    duplicate_hashes = sorted(
        value for value, count in Counter(r["receipt_hash"] for r in records).items() if count > 1
    )
    event_keys = [
        f"{r['runtime_id']}|{r['database']}|{r['ingest_id']}|{r['timestamp']}"
        for r in records
    ]
    duplicate_events = sorted(
        value for value, count in Counter(event_keys).items() if count > 1
    )

    ordered = sorted(records, key=lambda item: (item["timestamp"], item["path"]))
    ...
    return {
        ...
        "duplicate_runtime_events": duplicate_events,
        "errors": errors,
    }
